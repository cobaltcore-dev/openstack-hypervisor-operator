...
func (r *EvictionReconciler) liveMigrate(ctx context.Context, uuid string, hypervisorName string) error {
	log := logger.FromContext(ctx)
...
func (r *EvictionReconciler) coldMigrate(ctx context.Context, uuid string, hypervisorName string) error {
	log := logger.FromContext(ctx)
